...
final class TripLocalDataSourceImpl: TripLocalDataSource {
    ...
    func saveTrip(_ trip: Trip) -> Single<Trip> {
        return Single.create { [weak self] observer in
            guard let self = self else {
                observer(.failure(NSError(domain: "TripLocalDataSource", code: -1)))
            ...
            do {
                ...
                // Return saved trip with new ObjectId
                let savedTrip = tripR.toDomain()
                print("✅ Trip saved: \(savedTrip.title) with ID: \(savedTrip.id)")
                observer(.success(savedTrip))
            } catch {
    ...
    func updateTrip(_ trip: Trip) -> Completable {
        return saveTrip(trip)
            .asCompletable() // Upsert - ignore returned Trip
    }
